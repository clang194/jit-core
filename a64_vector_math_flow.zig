...
pub const Core64Methods = struct {
    ...
    pub fn runVectorWideningArithmetic(self: *Core64, word: u32) Core64Error!bool {
        ...
        const signed = masked == 0x0e200000 or masked == 0x0e201000 or masked == 0x0e202000 or masked == 0x0e203000 or signed_difference or signed_multiply_accumulate or signed_multiply_long;
        ...
        while (index < 8 / source_bytes) : (index += 1) {
            ...
            const left = if (widening_source_pair or absolute_difference or signed_multiply_accumulate or signed_multiply_long) if (signed) signExtendRuntime(raw_left, @intCast(u6, source_bits)) else raw_left else vectorElement(base, index, target_bytes);
