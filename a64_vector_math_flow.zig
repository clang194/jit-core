...
pub const Core64Methods = struct {
    ...
    pub fn runVectorAdd(self: *Core64, word: u32) Core64Error!bool {
        ...
        if (masked != 0x0e200400 and masked != 0x0e208400 and masked != 0x0e209400 and masked != 0x0e209c00 and masked != 0x2e200400 and masked != 0x2e208400 and masked != 0x2e209400) {
            return false;
        }

        const full = (word & 0x40000000) != 0;
        const size = @intCast(u2, (word >> 22) & 3);
        if (size == 3 and (masked == 0x0e200400 or masked == 0x0e209400 or masked == 0x0e209c00 or masked == 0x2e200400 or masked == 0x2e209400 or !full)) {
            return error.ReservedInstruction;
        }
        ...
        const result = if (masked == 0x0e209400 or masked == 0x2e209400) blk: {
            ...
            .low = if (masked == 0x0e200400) signedHalvingAddVectorLanes(left.low, right.low, lane) else if (masked == 0x2e200400) halvingAddVectorLanes(left.low, right.low, lane) else if (masked == 0x0e208400) addVectorLanes(left.low, right.low, lane) else if (masked == 0x0e209c00) multiplyVectorLanes(left.low, right.low, lane) else subtractVectorLanes(left.low, right.low, lane),
            .high = if (full) if (masked == 0x0e200400) signedHalvingAddVectorLanes(left.high, right.high, lane) else if (masked == 0x2e200400) halvingAddVectorLanes(left.high, right.high, lane) else if (masked == 0x0e208400) addVectorLanes(left.high, right.high, lane) else if (masked == 0x0e209c00) multiplyVectorLanes(left.high, right.high, lane) else subtractVectorLanes(left.high, right.high, lane) else 0,
        };
        self.state.writeVector(vectorRegFromWord(word), result);
        self.state.pc +%= 4;
        return true;
