...
pub const Core64Methods = struct {
    ...
    pub fn runFloatToInteger(self: *Core64, word: u32) Core64Error!bool {
        const masked = word & 0x7f3ffc00;
        if (masked != 0x1e200000 and masked != 0x1e380000 and masked != 0x1e390000) {
            return false;
        }
        ...
        const result = try fixedFromFloat(self, mode == 1, input, if (wide) @as(usize, 64) else @as(usize, 32), 0, masked == 0x1e390000, if (masked == 0x1e200000) .nearest else .zero);
        self.writeSized(wide, regFromWord(word), result, false);
        self.state.pc +%= 4;
        return true;
    }
