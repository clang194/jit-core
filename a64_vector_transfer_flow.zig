...
const expandVectorHalfFloatImmediate = a64_immediate_vectors.expandVectorHalfFloatImmediate;
const expandVectorImmediate = a64_immediate_vectors.expandVectorImmediate;
const lowestSetBit5 = a64_immediate_vectors.lowestSetBit5;
const signExtendRuntime = a64_immediate_vectors.signExtendRuntime;
const spreadVectorElement = a64_immediate_vectors.spreadVectorElement;
...
const a64_crypto_vectors = @import("a64_crypto_vectors.zig");
const decryptAesVector = a64_crypto_vectors.decryptAesVector;
const encryptAesVector = a64_crypto_vectors.encryptAesVector;
const mixAesVector = a64_crypto_vectors.mixAesVector;
...
const a64_vector_shift = @import("a64_vector_shift.zig");
const widenShiftLeftVectorHalf = a64_vector_shift.widenShiftLeftVectorHalf;
const a64_vector_table = @import("a64_vector_table.zig");
const lookupVectorBytes = a64_vector_table.lookupVectorBytes;
usingnamespace @import("a64_count_bits.zig");
const a64_memory_bits = @import("a64_memory_bits.zig");
const regFromWord = a64_memory_bits.regFromWord;
const vectorRegFromWord = a64_memory_bits.vectorRegFromWord;
...
pub const Core64Methods = struct {
    ...
    pub fn runScalarIntegerToFloat(self: *Core64, word: u32) Core64Error!bool {
        ...
        const result = if (double)
            if (signed) signedDoublewordToFloat64(source, 0) else unsignedDoublewordToFloat64(control, source, 0)
        else
            @as(u64, if (signed) signedWordToFloat32(@intCast(u32, source), 0) else unsignedWordToFloat32(@intCast(u32, source), 0));
        self.state.writeVector(vectorRegFromWord(word), a64_state.VectorValue{ .low = @as(u64, result), .high = 0 });
        self.state.pc +%= 4;
        return true;
    }
