...
pub fn rootStep16(left: u16, right: u16, control: float_control.Control, status: *float_status.FloatStatus) float_exception.FloatExceptionError!u16 {
    ...
    const right_zero = right_analysis.kind == .zero;
    if ((left_infinity and right_zero) or (left_zero and right_infinity)) {
        return float_format.Binary16.finite(false, -1, 3);
    }
    if (left_infinity or right_infinity) {
