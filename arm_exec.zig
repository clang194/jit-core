...
fn isHalfSaturatingMove(word: u32) bool {
...
fn isUnsignedSaturatingAddSubHalves(word: u32) bool {
    return (word & 0x0ff00ff0) == 0x06600f30 and armCondition(word) != null;
}

fn isUnsignedSaturatingSubAddHalves(word: u32) bool {
    return (word & 0x0ff00ff0) == 0x06600f50 and armCondition(word) != null;
}

fn isSignedSaturatingAddSubHalves(word: u32) bool {
    return (word & 0x0ff00ff0) == 0x06200f30 and armCondition(word) != null;
}

fn isSignedSaturatingSubAddHalves(word: u32) bool {
...
fn isUnsignedHalvingAddHalves(word: u32) bool {
...
fn isUnsignedHalvingAddSubHalves(word: u32) bool {
    return (word & 0x0ff00ff0) == 0x06700f30 and armCondition(word) != null;
}

fn isUnsignedHalvingSubAddHalves(word: u32) bool {
    return (word & 0x0ff00ff0) == 0x06700f50 and armCondition(word) != null;
}

fn isSignedHalvingAddSubHalves(word: u32) bool {
    return (word & 0x0ff00ff0) == 0x06300f30 and armCondition(word) != null;
}

fn isSignedHalvingSubAddHalves(word: u32) bool {
...
fn coprocessorOp(word: u32) ?CoprocessorOp {
...
fn runCoprocessor(word: u32, state: *arm_state.MachineState, hooks: arm_state.HostHooks, pc: u32) ArmStepError!void {
...
fn runCoprocessorCommand(word: u32, state: *arm_state.MachineState, hook: arm_state.CoprocessorHooks) ArmStepError!void {
...
fn runCoprocessorSendWord(word: u32, state: *arm_state.MachineState, hook: arm_state.CoprocessorHooks) ArmStepError!void {
...
fn runCoprocessorSendPair(word: u32, state: *arm_state.MachineState, hook: arm_state.CoprocessorHooks) ArmStepError!void {
...
fn runCoprocessorGetWord(word: u32, state: *arm_state.MachineState, hook: arm_state.CoprocessorHooks) ArmStepError!void {
...
fn runCoprocessorGetPair(word: u32, state: *arm_state.MachineState, hook: arm_state.CoprocessorHooks) ArmStepError!void {
    const low = armReg(word >> 12);
    const high = armReg(word >> 16);
...
fn runCoprocessorBlock(word: u32, state: *arm_state.MachineState, hook: arm_state.CoprocessorHooks, pc: u32, load: bool) ArmStepError!void {
...
fn coprocessorWord(word: u32) arm_state.CoprocessorWord {
...
fn coprocessorPair(word: u32) arm_state.CoprocessorPair {
...
fn runHalfSaturatingMove(word: u32, state: *arm_state.MachineState, pc: u32) ArmStepError!void {
    const dest = armReg(word >> 12);
    const source = armReg(word);
    if (dest == .pc or source == .pc) {
        return error.Unpredictable;
...
fn runUnsignedSaturatingMixedHalves(word: u32, state: *arm_state.MachineState, pc: u32, add_sub: bool) ArmStepError!void {
    const left_reg = armReg(word >> 16);
    const dest = armReg(word >> 12);
    const right_reg = armReg(word);
    if (left_reg == .pc or dest == .pc or right_reg == .pc) {
...
fn runSignedSaturatingMixedHalves(word: u32, state: *arm_state.MachineState, pc: u32, add_sub: bool) ArmStepError!void {
    const left_reg = armReg(word >> 16);
    const dest = armReg(word >> 12);
    const right_reg = armReg(word);
    if (left_reg == .pc or dest == .pc or right_reg == .pc) {
...
fn runUnsignedHalvingAddHalves(word: u32, state: *arm_state.MachineState, pc: u32) ArmStepError!void {
    const left_reg = armReg(word >> 16);
    const dest = armReg(word >> 12);
    const right_reg = armReg(word);
    if (left_reg == .pc or dest == .pc or right_reg == .pc) {
...
fn runUnsignedHalvingMixedHalves(word: u32, state: *arm_state.MachineState, pc: u32, add_sub: bool) ArmStepError!void {
    const left_reg = armReg(word >> 16);
    const dest = armReg(word >> 12);
    const right_reg = armReg(word);
    if (left_reg == .pc or dest == .pc or right_reg == .pc) {
...
fn runSignedHalvingMixedHalves(word: u32, state: *arm_state.MachineState, pc: u32, add_sub: bool) ArmStepError!void {
    const left_reg = armReg(word >> 16);
    const dest = armReg(word >> 12);
    const right_reg = armReg(word);
    if (left_reg == .pc or dest == .pc or right_reg == .pc) {
...
fn runUnsignedHalvingSubBytes(word: u32, state: *arm_state.MachineState, pc: u32) ArmStepError!void {
    const left_reg = armReg(word >> 16);
    const dest = armReg(word >> 12);
    const right_reg = armReg(word);
    if (left_reg == .pc or dest == .pc or right_reg == .pc) {
...
fn runUnsignedHalvingSubHalves(word: u32, state: *arm_state.MachineState, pc: u32) ArmStepError!void {
    const left_reg = armReg(word >> 16);
    const dest = armReg(word >> 12);
    const right_reg = armReg(word);
    if (left_reg == .pc or dest == .pc or right_reg == .pc) {
...
fn runSignedHalvingSubBytes(word: u32, state: *arm_state.MachineState, pc: u32) ArmStepError!void {
    const left_reg = armReg(word >> 16);
    const dest = armReg(word >> 12);
    const right_reg = armReg(word);
    if (left_reg == .pc or dest == .pc or right_reg == .pc) {
...
fn runSignedHalvingSubHalves(word: u32, state: *arm_state.MachineState, pc: u32) ArmStepError!void {
    const left_reg = armReg(word >> 16);
    const dest = armReg(word >> 12);
    const right_reg = armReg(word);
    if (left_reg == .pc or dest == .pc or right_reg == .pc) {
...
fn runUnsignedWrappingAddBytes(word: u32, state: *arm_state.MachineState, pc: u32) ArmStepError!void {
    const left_reg = armReg(word >> 16);
    const dest = armReg(word >> 12);
    const right_reg = armReg(word);
    if (left_reg == .pc or dest == .pc or right_reg == .pc) {
...
fn coprocessorReg(value: u32) arm_state.CoprocessorReg {
...
fn clampUnsignedHalfWord(value: i32) u16 {
    if (value > 65535) {
        return 0xffff;
    }
    if (value < 0) {
